...
def serialize_tag(tag):
    # print("len(Post.objects.filter(tags=tag))",len(Post.objects.filter(tags=tag)))
...
def index(request):
    ...
    most_popular_posts_ids=[post.id for post in most_popular_posts]
    posts_with_tags = Post.objects.filter(id__in = most_popular_posts_ids).annotate(
                          posts_count=Count('posts'))
    ids_and_tags = posts_with_tags.values_list('id', 'posts_count')
    count_for_id = dict(ids_and_tags)

    for post in most_popular_posts:
        post.posts_count = count_for_id[post.id]
...
def tag_filter(request, tag_title):
    ...
    # tags = Tag.objects.annotate(post_count=Count('posts'))
    # most_popular_tags_1 = Tag.popular().annotate(post_count=Count('posts'))

    # # print("what i have in tags",tags)
    # 

    # print("what i have in serialize_tag(tag)",serialize_tag(tag))
    # most_popular_tags = Tag.objects.annotate(post_count=Count('posts')) 
    most_popular_tags = Tag.objects.annotate(post_count=Count('posts')) 


    print("what i have in most_popular_tags",most_popular_tags)
    # most_popular_tags_ids=[tag.id for tag in most_popular_tags]
    # posts_with_tags= Post.objects.filter(id__in = most_popular_tags_ids).annotate(
    #                       post_count=Count('tags'))

    # ids_and_tags = posts_with_tags.values_list('id', 'post_count')

    for i in most_popular_tags:

    # # test = len(Post.objects.filter(tags=tag))
        print("what i have in most_popular_tags", (i.post_count))
        print("what i have in ш", (dir(i )))
    # for i in most_popular_tags_1:

    # # test = len(Post.objects.filter(tags=tag))
    #     print("what i have in most_popular_tags_1", (i.post_count))
    # # print("what i have in posts_with_tags",posts_with_tags)
    # print("what i have in ids_and_tags",ids_and_tags)1
    # print("what i have in serialize_tag(tag)",serialize_tag(tag))
